...
# Hong Qin
# 
# 041903 mv countT to Nucleotide.pm
############################################################
...
############################################################
# Date:023008 	Author: Hong Qin
# Usage:  $flag = codon2aa_with_table( $codon, \%genetic_code ); 
# Description: translate codon based on the input genetic code
#		return 1 upon success, 0 upon failure
# Note: everything is in uppercase (or the same case);
# Tested in ?
# Used in ?

sub codon2aa_with_table { 
 my ($codon, $rf) = @_;  my $debug = 1;
 print "useless subroutine?\n"; exit;
 return $rf->{$codon};
}
...
############################################################
# Date:070104 	Author: Hong Qin
# Usage:  $flag = count_by_input_hash($line,\%in_hash );  
# Description: count the occurence of the keys of  %in_hash in $line
#		return 1 upon success, 0 upon failure
# Note: everything is in uppercase (or the same case);
# Calls Codon:: @triplets = line2triplets( \$line );
# Tested in ?
# Used in codonpl/sce_IVA_major_codon_ATvsGC_ratio.00.pl

sub count_by_input_hash {
  use strict; use warnings; my $debug = 0;
  my ( $inline, $r_h ) = @_;

  my $line = uc $inline;
  my @triplets = line2triplets( \$line );

  foreach my $token ( @triplets ) {
    if ( exists( $r_h->{$token} ) ) { $r_h->{$token} ++ ; 
	if ($debug) { print "$token\t"; }
    }
  }

  if ($debug) { showHashTable( $r_h ); }
  return 1;
}
...
############################################################
# Date:Nov 11, 2003	Author: Hong Qin
# Usage:  my $peptide = dna2peptide_table11 ( $dna );
# Description: Use NCBI table 11 for the transaltion
# Calls BeginPerlBioinfo::codon2aa
# Tested in test/codon/table11.pl
# Used in cds_to_protein.00.pl

my $_str_first_codons_table_11 ="ATT ATC ATA ATG TTG CTG GTG";

sub dna2peptide_table11 {
 use strict; use warnings; 
 my($dna) = @_;  
 my $protein = '';
 my %h_first_codons_tab11 = ();

 my @tokens = split ( /\s+/, $_str_first_codons_table_11 );
 foreach my $token ( @tokens ) {
   $token = uc $token;
   $h_first_codons_tab11{ $token } ++;
 }

 my $first_codon = substr($dna, 0, 3);
 if ( exists $h_first_codons_tab11{ $first_codon } ) {
    	$protein .= "M";
 } else {
	$protein .= codon2aa(  $first_codon );
 }

 for(my $i=3; $i < (length($dna) - 2) ; $i += 3) {
...
############################################################
#Usage: $num_of_T = countT($dna_seq) 041903 moved to Nucleotide.pm
